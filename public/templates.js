...
var templates = {
	...
	action: '<div class="btn btn-large btn-{{type}}" {{#loading}}data-loading-text="{{loading}}"{{/loading}} {{#href}}href="#{{href}}" role="button" data-toggle="modal"{{/href}} {{#onclick}}onclick="{{onclick}}"{{/onclick}} {{#disabled}}disabled="disabled"{{/disabled}}>{{#icon}}<span class="glyphicon glyphicon-{{icon}}"></span> {{/icon}}{{label}}</div>',
	...
	tab: '<li {{#active}}class="active"{{/active}} {{#disabled}}class="disabled"{{/disabled}}><a href="#tab-{{id}}" {{^disabled}}data-toggle="{{type}}"{{/disabled}} {{#onclick}}onclick="{{onclick}}"{{/onclick}}>{{label}}</a></li>',
