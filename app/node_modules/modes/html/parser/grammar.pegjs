...

{
	...
	Node.addHook(function(node) {
		if (orphanCandidates.indexOf(node.type.element) >= 0) {
			node.flag('possibly-orphan');
		}
	});
}
...
block = open:opening/*?*/ ws0:__ elements:(elementList __)? close:closing? {
	...
	// } else {
	// 	node.flag('error');
	// 	errors.push('Block has no open tag');
	// 	node.set('error', errors);
	// }
...
closing = opening:closingOpening ws0:__ id:id ws1:__ closing:closingClosing? {
	...
	if (closing !== "") {
		...
		node.flag('error');
		node.set('error', 'Closing tag not closed');
	}
...
attribute = key:id value:(__ attributeAssignmentOperator __ attrValue?)? {
	...
	if (value !== "") {
		...
		var attrValue = value[3];
		if (attrValue !== "") {
			node.add('value', attrValue);
		} else {
			node.flag('error');
...
doubleQuoteString = opening:doubleQuoteStringQuote raw:doubleQuoteStringContent? closing:doubleQuoteStringQuote? {
	...
	if (closing !== "") {
		...
		node.flag('error');
		node.set('error', 'String not closed (missing souble quote)');
	}
